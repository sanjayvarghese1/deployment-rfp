...
export default function CompaniesPage() {
  ...
  useEffect(() => {
    void (async () => {
      const { data, error } = await supabase.from("users").select("*").eq("user_type", "vendor");
      if (error) {
        console.warn("Vendors load failed:", error);
        setCompanies([]);
        return;
      }
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        router.push("/login");
      } else if (profile && profile.user_type === "vendor") {
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="card !p-0 overflow-hidden mb-6">
          ...
          <div className="relative flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h1 className="text-2xl font-bold text-[#000000]">Bidders</h1>
              </div>
              <p className="text-sm text-[#333333]">
                Discover and connect with {companies.length} {companies.length === 1 ? "bidder" : "bidders"} in the network
              </p>
            </div>
        ...
        <div className={`flex items-center gap-3 px-5 py-3.5 border-b transition-colors ${searchFocused ? "border-[var(--primary)]" : "border-[var(--divider)]"}`}>
          ...
          <input
            ...
            placeholder="Search bidders by name, industry, or description..."
      ...
      {filtered.length === 0 && (
        <div className="card text-center py-16">
          ...
          <p className="font-semibold text-[var(--foreground)]">No bidders found</p>
