...
function getSupabaseEnvError(): Error {
  return new Error(
...
function createMissingSupabaseClient() {
  const throwEnvError = () => {
    throw getSupabaseEnvError();
  };

  // Proxy ensures any property or method access fails with a clear runtime message,
  // while avoiding module-import crashes during next build.
  const proxy = new Proxy(throwEnvError as unknown as Record<string, unknown>, {
    apply: () => {
      throw getSupabaseEnvError();
    },
    get: () => proxy,
