...
function buildFullHtml(
  ...
  const css = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: ${fontFamily}; font-size: 12pt; line-height: 1.72; color: #1a1a1a; }
    ...
    .toc-item { display: flex; align-items: baseline; padding: 7px 0; border-bottom: 1px dotted #ccc; font-size: 12.5pt; }
    .toc-num { font-weight: 700; color: ${colors.primary}; min-width: 28px; }
    .toc-label { flex: 1; }

    .section { page-break-before: always; padding: 50px 42px 50px 42px; }
    .section h2 { font-size: 20pt; color: ${colors.primary}; border-bottom: 2px solid ${colors.primary}; padding-bottom: 7px; margin-bottom: 14px; }
    h3 { font-size: 14pt; color: ${colors.headerBg}; margin: 18px 0 9px; font-weight: 700; }
    h4 { font-size: 13pt; color: ${colors.secondary}; margin: 14px 0 7px; font-weight: 700; }
    h5 { font-size: 12pt; color: ${colors.primary}; margin: 12px 0 7px; font-weight: 700; }
    ...
    .data-table { width: 100%; border-collapse: collapse; margin: 18px 0; font-size: 11pt; }
    ...
    .info-box { background: #eff6ff; border-left: 4px solid ${colors.primary}; padding: 14px 18px; margin: 14px 0; border-radius: 0 4px 4px 0; font-style: italic; font-size: 12.5pt; }
    ...
    .chart-title { font-size: 13.5pt; font-weight: 700; color: ${colors.headerBg}; margin-bottom: 12px; margin-top: 0; }
    .bar-chart { display: grid; gap: 12px; margin: 14px 0; }
    .bar-row { display: flex; align-items: center; gap: 10px; }
    .bar-label { min-width: 150px; font-size: 10.5pt; font-weight: 700; color: #1f2937; }
    ...
    .bar-value { min-width: 52px; text-align: right; font-size: 10.5pt; color: #4b5563; font-weight: 700; }
...
export async function generateRfpPdf(
  ...
  const forcePdfShift = process.env.RFP_FORCE_PDFSHIFT !== "false";

  try {
    const apiKey = process.env.PDFSHIFT_API_KEY;
    if (!apiKey) {
      if (forcePdfShift) {
        throw new Error("PDFSHIFT_API_KEY is missing. Configure it to generate RFP PDFs.");
      }
    ...
    if (forcePdfShift) {
      ...
      throw new Error(`PDFShift rendering failed: ${message}`);
    }
