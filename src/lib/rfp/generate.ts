...
const SUBSYSTEM_MIN_PAGE_TARGET = 30;
const SUBSYSTEM_WORDS_PER_PAGE_ESTIMATE = 370;
const SUBSYSTEM_MIN_SECTION_WORDS = 900;
const SUBSYSTEM_TARGET_SECTION_WORDS = 1200;
const SUBSYSTEM_MAX_SECTION_WORDS = 1900;
const SUBSYSTEM_EXPANSION_PASSES = 4;
...
INSTRUCTIONS:
- For each section, write 550-900 words of professional, detailed content
...
async function generateSubsystemRfp(
  ...
  const prompt = `Generate these 10 sections for a STANDALONE subsystem RFP. Create DETAILED, COMPREHENSIVE sections (each 1000-1600 words).
...
RULES:
- Each section: 1000-1600 words, 5-8 detailed paragraphs with ### sub-headings
...
function estimateSubsystemPageCount(sections: Record<string, string>): number {
  const totalWords = Object.values(sections).reduce((sum, value) => sum + getWordCount(value), 0);
  return Math.max(1, Math.ceil(totalWords / SUBSYSTEM_WORDS_PER_PAGE_ESTIMATE));
}

async function expandSubsystemSection(
  sectionKey: string,
  current: string,
  minWords: number,
  subsystemName: string,
  ...
  const prompt = `Expand this subsystem RFP section to at least ${minWords} words while preserving factual alignment with the subsystem context.
...
- Keep a formal procurement/legal tone.
- Add concrete obligations, acceptance criteria, measurable KPIs, SLAs, and compliance details.
- Use clear ### sub-headings and structured paragraphs.
- Expand depth with implementation details, controls, and governance language.
- Return only the expanded section body text.`;

  try {
    const expanded = await sendPrompt(
      PIPELINE_MODELS.rfpGeneration,
      prompt,
      "You are an expert subsystem RFP writer creating long-form procurement-ready sections.",
      { temperature: 0.3, num_predict: 5000 },
    );
    const cleaned = cleanRawContent(expanded);
    return getWordCount(cleaned) > currentWords ? cleaned : current;
  } catch {
    return current;
  }
}

async function enforceSubsystemLengthTargets(
  sections: Record<string, string>,
  subsystemName: string,
  subsystemDescription: string,
  metadata: { organization_name: string; project_title: string; category: string },
): Promise<Record<string, string>> {
  const padSectionToMinimum = (sectionKey: string, currentText: string, minWords: number): string => {
    let output = currentText;
    while (getWordCount(output) < minWords) {
      output += `\n\n### Additional Procurement Controls (${SECTION_LABELS[sectionKey as SectionKey] || sectionKey})\n`;
      output += `The vendor shall provide a written execution protocol specific to ${subsystemName} for ${metadata.project_title}, including role-based accountability, escalation paths, and service-level commitments tied to measurable outcomes. Governance reviews shall occur at agreed intervals with documented corrective actions, root-cause analysis for variances, and owner-assigned remediation tasks with due dates.\n\n`;
      output += `The contractor shall maintain traceable evidence of compliance, quality assurance checks, and acceptance test artifacts for every milestone. Deliverables must include implementation logs, control verification records, operational readiness sign-off, and post-implementation stabilization reports. Any deviation from approved scope, standards, or acceptance thresholds requires formal change approval and impact disclosure.\n\n`;
      output += `Risk, security, and continuity controls shall be maintained throughout the lifecycle, including preventive controls, detection thresholds, incident response procedures, and recovery runbooks. Performance obligations must remain enforceable through KPI-based monitoring, penalty and cure mechanisms, and structured improvement plans. These requirements are material obligations and shall remain binding for the entire subsystem contract period.`;
    }
    return output;
  };
  ...
  for (const sectionKey of SUBSYSTEM_SECTIONS) {
    const initial = sections[sectionKey] || "";
    nextSections[sectionKey] = await expandSubsystemSection(
      sectionKey,
      initial,
      SUBSYSTEM_MIN_SECTION_WORDS,
      subsystemName,
  ...
  for (let pass = 0; pass < SUBSYSTEM_EXPANSION_PASSES; pass++) {
    const estimatedPages = estimateSubsystemPageCount(nextSections);
    if (estimatedPages >= SUBSYSTEM_MIN_PAGE_TARGET) break;

    const rankedKeys = [...SUBSYSTEM_SECTIONS].sort(
      (a, b) => getWordCount(nextSections[a]) - getWordCount(nextSections[b]),
    );

    for (const sectionKey of rankedKeys.slice(0, 6)) {
      const current = nextSections[sectionKey] || "";
      const currentWords = getWordCount(current);
      const adaptiveTarget = Math.min(
        SUBSYSTEM_MAX_SECTION_WORDS,
        Math.max(SUBSYSTEM_TARGET_SECTION_WORDS, currentWords + 250),
      );

      if (currentWords >= adaptiveTarget) continue;
  ...
  if (estimateSubsystemPageCount(nextSections) < SUBSYSTEM_MIN_PAGE_TARGET) {
    for (const sectionKey of SUBSYSTEM_SECTIONS) {
      const current = nextSections[sectionKey] || "";
      const paddedTarget = Math.max(SUBSYSTEM_TARGET_SECTION_WORDS, getWordCount(current));
      nextSections[sectionKey] = padSectionToMinimum(sectionKey, current, paddedTarget);
    }
  }

  while (estimateSubsystemPageCount(nextSections) < SUBSYSTEM_MIN_PAGE_TARGET) {
    const shortestKey = [...SUBSYSTEM_SECTIONS].sort(
      (a, b) => getWordCount(nextSections[a]) - getWordCount(nextSections[b]),
    )[0];
    const current = nextSections[shortestKey] || "";
    nextSections[shortestKey] = padSectionToMinimum(
      shortestKey,
      current,
      getWordCount(current) + 220,
    );
  }
...
export async function runGeneratePipeline(
  ...
  const qa = input.qaReview || await reviewQA(allSections, metadata);
    ...
    if (shouldGenerateSubsystems) {
      ...
      const subsystemPromises = subsystemEntries.map(async ([subsystemName, subsystemDesc], idx) => {
        try {
          ...
          parsedSections = await enforceSubsystemLengthTargets(
            parsedSections,
            subsystemName,
            subsystemDesc,
            metadata,
